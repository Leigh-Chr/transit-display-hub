...
class DisplayStateCalculatorTest {
    ...
    class CalculateForStop {
        ...
        @Test
        @DisplayName("returns display state with stop and line info")
        void returnsDisplayStateWithStopInfo() {
            when(stopRepository.findById(testStopId)).thenReturn(Optional.of(testStop));
            when(timedEntryRepository.findByStopIdAndTimeAfter(eq(testStopId), any(LocalTime.class)))
                    .thenReturn(List.of());
            when(messageRepository.findActiveMessagesForStop(any(Instant.class), eq(testLineId), eq(testStopId)))
                    .thenReturn(List.of());
    ...
    class ArrivalsFiltering {
        ...
        void filtersPassedArrivals() {
            ...
            TimedEntry futureEntry = TestDataFactory.createTimedEntry(LocalTime.now().plusHours(1), testStop);
        ...
        void limitsToFiveArrivals() {
            ...
            List<TimedEntry> manyEntries = List.of(
                    TestDataFactory.createTimedEntry(LocalTime.of(8, 0), testStop),
                    TestDataFactory.createTimedEntry(LocalTime.of(8, 15), testStop),
                    TestDataFactory.createTimedEntry(LocalTime.of(8, 30), testStop),
                    TestDataFactory.createTimedEntry(LocalTime.of(8, 45), testStop),
                    TestDataFactory.createTimedEntry(LocalTime.of(9, 0), testStop),
                    TestDataFactory.createTimedEntry(LocalTime.of(9, 15), testStop),
                    TestDataFactory.createTimedEntry(LocalTime.of(9, 30), testStop)
            );
        ...
        void includesArrivalTime() {
            ...
            TimedEntry entry = TestDataFactory.createTimedEntry(arrivalTime, testStop);
        ...
        void includesLineInfoInArrivals() {
            TimedEntry entry = TestDataFactory.createTimedEntry(LocalTime.of(14, 30), testStop);
