...
/**
 * Base class for per-IP rate-limit filters on the {@code /api/auth/*}
 * surface. Subclasses pin the matched endpoint and the bucket sizing —
 * everything else (proxy trust, bucket cache, 429 response shape) lives
 * here so adding a new gated endpoint stays a ~15-line subclass.
 *
 * <p>In-memory (Bucket4j) — single-node only. Multi-node deployments
 * must swap the backing store to Hazelcast or Redis.
 */
@Slf4j
abstract class AuthIpRateLimitFilter extends OncePerRequestFilter {
     ...
     * cap is a defence-in-depth backstop for the same scenario.
     */
    ...
    /** True when {@code req} targets the endpoint this filter protects. */
    protected abstract boolean matches(HttpServletRequest req);

    /** Bucket capacity = refill amount per window. */
    protected abstract int maxAttempts();

    /** Refill window in minutes (also the {@code Retry-After} returned on 429). */
    protected abstract int windowMinutes();
